 ...
 * } item
 */
function injectBlock3(timeline, rand, item, mask) {
  let temp = []
  const welcome_trial = {
  ...
  for (let i = 0; i < item.list.length; i++) {
    ...
    const image_trial = {
      ...
      trial_duration: !isEnd ? block3ShowTime : (block3ShowTime + block3MaskTime),
    ...
    const response_trial = {
      ...
      on_finish: function (data) {
        ...
        data.diffTime = (data.endTime - data.startTime) + (block3ShowTime + block3MaskTime);
        ...
        //   }
        // }
        if (data.trial === item.list.length) {
          onCredamoEndTrialFinish(jsPsych.data.get().csv());
        }
        // if (
        //   data.trial === item.list.length &&
        //   data.group_id &&
        //   data.sample_id
        // ) {
        //   window.location = `https://ttq.qq.com/api/collect/sample_callback?group_id=${data.group_id}&sample_id=${data.sample_id}&status=1`;
        // }
      },
    };
  ...
  timeline.push({
    ...
    conditional_function: function () {
      ...
      const ch2 = jsPsych.data
        .get()
        .filter({ block: 2, 图片: "测谎图" })
        .last(1)
        .values()[0];
      if (ch2 && ch2.response === "no") {
        return false;
      }
