...
// 不需要 SEO，但加个基本 title
useHead({
  title: 'wj-html-test | iframe 通信测试',
})
...
const stepLabel = computed(() => {
  if (currentStep.value === 1) return '第1步 - 选择水果'
...
function sendPostMessage(payload: Record<string, unknown>) {
  console.log('[wj-html-test] 发送 postMessage，payload:', JSON.stringify(payload), 'targetOrigin: *', '时间:', getTimestamp())
  window.parent.postMessage(payload, '*')
  lastPayload.value = payload
  addLog('send', '[postMessage]', '已发送', payload)
}

function handleSelect(key: '水果' | '蔬菜', value: string) {
  console.log(`[wj-html-test] 用户选择 ${key} = ${value}`)
  addLog('info', '[用户操作]', `选择了 ${key}: ${value}`)

  if (key === '水果') {
    // 第一步：上报水果
    const payload = { 水果: value }
    console.log('[wj-html-test] 准备发送第一步 payload:', JSON.stringify(payload))
    sendPostMessage(payload)
    currentStep.value = 2
    addLog('info', '[流程]', '进入第二步 - 选择蔬菜')
    console.log('[wj-html-test] 进入第二步，等待用户选择蔬菜')
  } else {
    // 第二步：上报蔬菜
    const vegPayload = { 蔬菜: value }
    ...
    console.log('[wj-html-test] 已发送结束关键词，本题作答完毕')
  }
}
...
function handleReset() {
  ...
  console.log('[wj-html-test] 页面已重置，回到第一步')
...
onMounted(() => {
  ...
  if (!isInIframe.value) {
    console.warn('[wj-html-test] 当前不是在 iframe 中运行，postMessage 将发送到自身 window')
    addLog('warn', '[环境检测]', '非 iframe 环境，postMessage 无接收方')
  } else {
    console.info('[wj-html-test] 检测到 iframe 环境，postMessage 将发送到 parent')
    addLog('info', '[环境检测]', 'iframe 环境就绪')
  }

  console.log('[wj-html-test] 页面初始化完成，等待用户选择第一步')
