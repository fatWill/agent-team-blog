          ...
          <table class="w-full min-w-[640px] text-sm">
            <thead>
              <tr class="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800/80">
                <th class="whitespace-nowrap px-3 py-3 text-left font-semibold text-gray-600 dark:text-gray-300 sm:px-4">序号</th>
                <th class="whitespace-nowrap px-3 py-3 text-left font-semibold text-gray-600 dark:text-gray-300 sm:px-4">分类</th>
                <th class="whitespace-nowrap px-3 py-3 text-left font-semibold text-gray-600 dark:text-gray-300 sm:px-4">项目名称</th>
                <th class="whitespace-nowrap px-3 py-3 text-right font-semibold text-gray-600 dark:text-gray-300 sm:px-4">预算金额</th>
                <th class="whitespace-nowrap px-3 py-3 text-right font-semibold text-gray-600 dark:text-gray-300 sm:px-4">实际支出</th>
                <th class="whitespace-nowrap px-3 py-3 text-right font-semibold text-gray-600 dark:text-gray-300 sm:px-4">差额</th>
                <th class="whitespace-nowrap px-3 py-3 text-left font-semibold text-gray-600 dark:text-gray-300 sm:px-4">责任人/备注</th>
              </tr>
            </thead>
            <tbody>
              <template v-for="(category, catIdx) in budgetData" :key="category.name">
                <tr
                ...
                >
                  <!-- 序号 -->
                  <td class="whitespace-nowrap px-3 py-2.5 text-gray-400 dark:text-gray-500 sm:px-4">
                    {{ getGlobalIndex(catIdx, itemIdx) }}
                  </td>
                  <!-- 分类（合并单元格） -->
                  <td
                    v-if="itemIdx === 0"
                  ...
                  >
                    <div class="flex items-center gap-2">
                      <span
                        class="inline-block h-8 w-1 rounded-full"
              ...
              <!-- 分类小计行 -->
              <template v-for="category in budgetData" :key="'subtotal-' + category.key">
                <tr class="border-b border-gray-200 dark:border-gray-600" :class="categorySubtotalBgClass(category.key)">
                  <td class="px-3 py-2.5 sm:px-4" />
                  <td class="px-3 py-2.5 sm:px-4">
                    <span class="text-xs font-bold" :class="categorySubtotalTextClass(category.key)">{{ category.name }}小计</span>
                  </td>
                  <td class="px-3 py-2.5 sm:px-4" />
              ...
              <!-- 总计行 -->
              <tr class="border-t-2 border-gray-300 bg-gray-100 dark:border-gray-600 dark:bg-gray-700">
...
/** 获取全局序号 */
function getGlobalIndex(catIdx: number, itemIdx: number): number {
  let idx = 0
  for (let i = 0; i < catIdx; i++) {
    idx += budgetData[i].items.length
  }
  return idx + itemIdx + 1
}
